...
def get_upbit_ticker():
    ...
    try:
        ...
        return data['trade_price'], data['signed_change_rate'], data['signed_change_price']
    except:
        return 0, 0, 0
...
def main():
    ...
    # 상단: 실시간 가격 및 자동 새로고침 (옵션)
    krw_price, change_rate, change_price = get_upbit_ticker()
    
    with st.spinner('차트 데이터를 불러오는 중입니다...'):
        df = get_crypto_data()
        fng_value, fng_class = get_fear_and_greed()
    
    if df.empty or krw_price == 0:
        st.error("데이터를 불러오지 못했습니다. 네트워크 상태를 확인해주세요.")
        return
    ...
    col1.metric("현재 비트코인 (KRW)", f"₩{krw_price:,.0f}", f"{change_rate*100:.2f}%")
    col2.metric("현재 비트코인 (USD)", f"${usd_price:,.2f}")
    col3.metric("RSI (14일)", f"{current_rsi:.2f}")
    col4.metric("공포/탐욕 지수", f"{fng_value} ({fng_class})")
    ...
    with left_col:
        ...
        if current_rsi > 70 and fng_value >= 75:
            ...
            reason.append("시장이 '극단적 공포' 상태로 패닉셀이 나왔으며, 저점 매수 기회일 수 있습니다.")
    ...
    with right_col:
        st.header("📋 실시간 매수/매도 호가창")
        if st.button("🔄 호가창 새로고침"):
            st.rerun()
    ...
    st.caption(f"데이터 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
